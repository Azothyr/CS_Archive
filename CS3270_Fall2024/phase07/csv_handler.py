...
class CSVHandler:
    ...
    def __init__(self, file_path: str):
        ...
        self.missing_columns_logged = False  # Track if a missing column has already been logged
        self.chunk_size = 1000  # Default chunk size for reading data

    def _read_data_in_chunks(self) -> DataFrame:
        """
        ...
        """
        try:
            log.debug(f"Reading file: {self.file_path}")
    ...
    def _process_chunk(self, chunk: DataFrame, columns: list[str] | None = None,
                       start_row: int | None = None, end_row: int | None = None,
                       filters: dict | None = None) -> DataFrame:
        """
        Process a chunk of data based on user inputs for columns, row ranges, and filters.

        Parameters:
            chunk (DataFrame): A chunk of data to process.
    ...
    def get_data(self, columns: list[str] | None = None, start_row: int | None = None, end_row: int | None = None,
        ...
        for chunk in self._read_data_in_chunks():
            # Process the chunk based on user inputs
            chunk = self._process_chunk(chunk, columns, start_row, end_row, filters)

            # Collect the processed chunk
            if not chunk.empty:
                data.append(chunk)

        # Use reduce to concatenate all chunks into a single DataFrame. Return empty DataFrame if no data
